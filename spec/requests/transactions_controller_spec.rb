...
RSpec.describe 'TransactionsControllers', type: :request do
  ...
  describe 'POST /create' do
    it 'returns http success' do
      post '/financial_transactions',
           params: { financial_transaction: { name: 'Transaction', amount: 100, user: @user,
                                              category_ids: [@category.id] } }
  ...
  describe 'PATCH /update' do
    it 'returns http success' do
      patch "/financial_transactions/#{FinancialTransaction.first.id}",
            params: { financial_transaction: { name: 'Transaction' } }
